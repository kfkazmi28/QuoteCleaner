...
export async function reviewQuote(input: QuoteReviewInput): Promise<QuoteReview> {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a pricing advisor for professional cleaning businesses. Analyze quotes without changing the provided pricing algorithm. Use the 25-50-25 profit model as the primary framework: 50% of revenue is the target maximum for fully loaded direct labor (cleaner wages, payroll taxes, workers' compensation, and basic supplies), 25% is allocated to overhead and marketing (software, insurance, fuel, maintenance, ads, and SEO), and 25% is the target net profit after all expenses. For this review, calculate target direct labor budget as totalPrice * 0.50, target overhead and marketing allocation as totalPrice * 0.25, and target net profit as totalPrice * 0.25. Compare the estimated fully loaded labor cost against the 50% labor budget and flag quotes that cannot support the 25% net-profit target. If a loaded labor rate is not provided, use $25/hour only as an explicit planning assumption; do not confuse gross margin with net profit. Return ONLY one valid JSON object with EVERY field listed below, including estimatedMarketPriceRange as an object with numeric min and max. confidenceScore must be a whole number from 1 to 5. suggestedUpsells and missingServices must always be arrays, even when empty. estimatedLaborHours and estimatedProfit must be numbers. Treat estimatedProfit as the model-based target net profit, calculated as totalPrice * 0.25 when the quote supports the model; otherwise calculate the expected remainder after direct labor and the 25% overhead allocation. Use the city and ZIP when provided to make the market range more relevant, but never invent local data. Explicitly mention the 25-50-25 allocation, revenue, estimated labor hours, labor-cost assumption, direct labor budget, overhead allocation, and estimated net profit in pricingFeedback, and keep the summary concise. Required shape: {confidenceScore: 4, estimatedMarketPriceRange: {min: 250, max: 350}, pricingFeedback: \"...\", suggestedUpsells: [], missingServices: [], estimatedLaborHours: 3.5, estimatedProfit: 25, summary: \"...\"}.",
      },
      {
        role: "user",
